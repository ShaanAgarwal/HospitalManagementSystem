...
class Hospital:
    def __init__(self, root):
        ...
        comNametablet=ttk.Combobox(DataframeLeft, textvariable=self.Nameoftablets,state="readonly", font = ("arial", 12, "bold"),
                                                                                    width = 33)
        comNametablet["values"] = ("Nice", "Corona Vaccine", "Acetaminiphen", "Adderall", "Amlodipine", "Ativan")
        ...
        lblFurtherinfoDateOfBirth=Label(DataframeLeft, font = ("arial", 12, "bold"), text = "Date Of Birth:", padx = 2, pady=6)
        lblFurtherinfoDateOfBirth.grid(row=7, column=2, sticky=W)
        txtFurtherinfoDateOfBirth=Entry(DataframeLeft, font = ("arial", 13, "bold"), textvariable=self.DateOfBirth, width=35)
        txtFurtherinfoDateOfBirth.grid(row = 7, column=3)
    ...
    def update_data(self):
        conn=mysql.connector.connect(host="localhost", username="root", password="<Password>", database="mydata")
        my_cursor=conn.cursor()
        my_cursor.execute("update hospital set Nameoftablets=%s, Nameoftablets=%s, dose=%s, Numbersoftablets=%s, lot=%s, issuedate=%s, expdate=%s, dailydose=%s, storage=%s, nhsnumber=%s, patientname=%s, DOB=%s, patientaddress=%s where Reference_No=%s",(
    ...
    def iPrectioption(self):
        ...
        self.txtPrescription.insert(END, "Lot:\t\t\t"+self.Issuedate.get()+"\n")
    ...
    def idelete(self):
        ...
        conn.close()
        self.fetch_data()
        messagebox.showinfo("Delete", "Patient has been delted successfully")
